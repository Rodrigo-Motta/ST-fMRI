...
def main(args):
    ...
    Variables:
        ...
        ICA_nodes: Number of ICA nodes.
        data_path: Root directory path where node timeseries data files are stored.
        T: Temporal length sequence, set to 1200.
        data: Numpy array initialized to zeros with shape (nb_subject, 1, T, ICA_nodes, 1) to store preprocessed timeseries data for each subject.
    ...
    for i in range(nb_subject):
        ...
        ## -----------
        ids.Gender.replace('M', 0, inplace=True)
        ids.Gender.replace('F', 1, inplace=True)
        ## --------
        ...
        ## ---------
        subject_string = format(int(ids.loc[i,'Subject']), '06d')
        ## --------
        ## -----------
        ids.Gender.replace('M', 0, inplace=True)
        ids.Gender.replace('F', 1, inplace=True)
        ...
        subject_string = format(int(ids.loc[i,'Subject']), '06d')
        ## --------
        filename = 'HCP_PTN1200/node_timeseries_{}/3T_HCP1200_MSMAll_d{}_ts2/'.format(ICA_nodes, ICA_nodes)+subject_string+'.txt'
        filepath = os.path.join(str(data_path),filename)
        print(filepath)
        if os.path.exists(filepath):
            full_sequence = pd.read_csv(filepath, delimiter = " ").to_numpy()[:110,:]
            full_sequence = pd.read_csv(filepath, delimiter = " ").to_numpy()[:110,:]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ICA', required=True, type=int, help='Number of ICA nodes.')
